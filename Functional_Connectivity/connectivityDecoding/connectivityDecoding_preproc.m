function TCS=connectivityDecoding_preproc(fMean_filename, sa_filename, ...
    fa_filename,maxRegionNidx, f_direc)
...
addpath('E:\CMST\MRIanalyses\EC-aMCI_N36\Resting-state\scripts\connectivityDecoding')  % add access to necessary tools
...
[myListofFiles,dontcare]=spm_select('List', f_direc, '^rf.*'); % make list of all realigned functional volumes - replace this by a programmatic call if preferred
disp('* Reading headers...');
tmp=pwd; % modif Elisa
cd(f_direc) % modif Elisa
...
%figure;plot(TCS(1:10,:)'); xlabel('volume number'); VK
